...
describe('Purchase Controllers Integration', () => {
  ...
  describe('POST /api/purchases', () => {
    it('should create a purchase with FRACTIONAL quantities (kg support), automatically INCREMENT product stock, and recalculate average costs', async () => {
      const payload = {
        ...
        items: [
          ...
          {
            product_id: productId,
            quantity: '0.25', // Fractional 2
            unit_cost: '200' // total cost of this item line = 50
          }
        ]
      };
      ...
      const details = await PurchaseDetail.find({ purchase_id: purchaseId }).sort({ quantity: -1 });
      expect(details).toHaveLength(2);
      expect(details[0].product_id.toString()).toBe(productId);
      expect(details[0].quantity.toString()).toBe('15.5');
      expect(details[1].quantity.toString()).toBe('0.25');

      // 2. STOCK DE SUCURSAL INCREMENTADO:
      const branchInv = await Inventory.findOne({ product_id: productId, branch_id: branchId });
      expect(branchInv.quantity.toString()).toBe('15.75');

      // 3. STOCK MOVEMENT CREADO
      const movements = await StockMovement.find({ product_id: productId, branch_id: branchId }).sort({ quantity_change: -1 });
      expect(movements).toHaveLength(2);
      expect(movements[0].type).toBe('PURCHASE');
      expect(movements[0].quantity_change.toString()).toBe('15.5');
      expect(movements[1].type).toBe('PURCHASE');
      expect(movements[1].quantity_change.toString()).toBe('0.25');
    ...
    it('should correctly rollback transaction (abortTransaction) and return 404 if product does not exist', async () => {
      ...
      const inventoryCount = await Inventory.countDocuments({ product_id: fakeProductId });
      expect(inventoryCount).toBe(0);
      const movementsCount = await StockMovement.countDocuments({ product_id: fakeProductId });
