...
export function usePOSKeyboard(options: UsePOSKeyboardOptions) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'Escape':
          ...
          // If a sale detail is open, close it via store
          const { viewedSaleId, closeModals: storeClose } = useSaleUIStore.getState();
          if (viewedSaleId) {
            storeClose();
            return;
          }
    ...
    isFormOpen, showHelp, isScannerOpen, isCartOpen, items, hasOpenShift,
    setShowHelp, setIsFormOpen, setIsScannerOpen,
    setIsCartOpen,
